...
def evaluate(rows):
    ...
    for minute in HORIZONS_MIN:
        ...
        for i in range(LOOKBACK_BARS, len(prices) - horizon):
            ...
            base_correct.append(1 if base == actual else 0)
            pers_correct.append(1 if pers == actual else 0)
            if pred == 1:
                cost_return += actual_move - FRICTION
            elif pred == -1:
                cost_return += -actual_move - FRICTION
        if not model_correct:
            raise RuntimeError(f'no evaluation folds for +{minute}m')
...
def main():
    ...
    deltas = [x['accuracy_delta_vs_majority'] for x in primary]
    costs = [x['cost_aware_return'] for x in primary]
    survives = all(
    ...
    result = {
        ...
        'snapshots': snapshots,
        'primary_summary': {
            'snapshot_deltas_vs_majority': deltas,
            'mean_delta_vs_majority': statistics.mean(deltas),
            'min_delta_vs_majority': min(deltas),
            'cost_aware_returns': costs,
            'all_snapshots_positive': survives,
        },
        'integrity': {
            'chronological': True,
            'future_features_used': False,
        ...
        'scientific_scope': 'Real 5-minute EUR/USD data are used because the audited predictor produces +5/+10/+20 minute forecasts. This evaluates predictive direction only and does not establish live execution quality or VERIFIED status.',
