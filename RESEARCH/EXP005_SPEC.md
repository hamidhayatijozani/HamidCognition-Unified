...
1. Canonical EXP-004 artifact must be accepted.
2. A falsification result relabeled as survival with matching forged promotion must be rejected.
...
EXP-005 PASS requires every adversarial mutation above to be rejected, the canonical artifact to be accepted, and the coherent survival path to be accepted.
...
The research pipeline is treated as:

`Execution → Observation → Interpretation → Validation → Promotion`

with Validation independently deriving and cross-checking Interpretation from evidence rather than trusting a result label emitted by the experiment runner.
