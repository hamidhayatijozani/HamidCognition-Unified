...
This experiment asks a narrower question than EXP-004: can the Evidence Gate
reject artifacts whose declared interpretation conflicts with their evidence?
The tests mutate one canonical result artifact and expect rejection/acceptance
according to pre-declared semantic rules.
"""
...
def main() -> int:
    ...
    expect_valid("canonical_falsification", canonical)

    cases = {}

    # Pure label forgery: negative evidence cannot be relabeled as survival.
    ...
    # Explicitly contradictory aggregate: all_snapshots_positive=true while deltas are negative.
    m = copy.deepcopy(canonical)
    m["primary_summary"]["all_snapshots_positive"] = True
    cases["forged_positive_aggregate"] = m

    # One positive snapshot must prevent a falsification verdict under the all-snapshots gate.
    m = copy.deepcopy(canonical)
    ...
    # A forged delta that does not equal model accuracy minus baseline must be rejected.
    m = copy.deepcopy(canonical)
    m["snapshots"]["snapshot_A"]["metrics"]["5"]["accuracy_delta_vs_majority"] = 0.01
    cases["forged_delta"] = m

    # Integrity claim cannot be asserted after removing disjointness.
    m = copy.deepcopy(canonical)
    m["integrity"]["snapshots_disjoint"] = False
    cases["integrity_disjointness_forgery"] = m

    # Future-feature contamination must invalidate the artifact even when labels remain unchanged.
    m = copy.deepcopy(canonical)
    m["snapshots"]["snapshot_B"]["metrics"]["5"]["integrity"]["future_features_used"] = True
    cases["future_feature_contamination"] = m

    # Missing/invalid source fingerprint is not acceptable evidence provenance.
    m = copy.deepcopy(canonical)
    ...
    # The gate must not be hard-coded to falsification: construct a coherent
    # synthetic survival artifact from the canonical structure and ensure it passes.
    ...
    report = {
        ...
        "result": "PASS",
        "interpretation": "Evidence validator rejected all declared adversarial mutations and accepted both coherent falsification and coherent survival paths.",
        "scope": "This validates the validator's tested semantic rules; it does not prove completeness against all possible adversarial inputs.",
    }
