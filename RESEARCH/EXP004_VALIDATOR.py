    ...
    if isinstance(snapshots, dict):
        for name in ("snapshot_A", "snapshot_B"):
            ...
            if not isinstance(snap, dict):
                ...
                continue
            raw_sha, obs, metrics = snap.get("raw_sha256"), snap.get("observations"), snap.get("metrics")
            ...
            if not (isinstance(raw_sha, str) and len(raw_sha) == 64): snapshot_integrity_ok = False
            if not (isinstance(obs, int) and obs >= 500): snapshot_integrity_ok = False
            ...
            if isinstance(metric_integrity, dict):
                ...
                if not (chronological and future_free and untuned): snapshot_integrity_ok = False
            else: snapshot_integrity_ok = False
            if all(isinstance(x, (int, float)) for x in (model_accuracy, baseline_accuracy, delta, cost)):
                ...
                primary_deltas.append(float(delta)); primary_costs.append(float(cost))
...
def assert_valid(payload: Dict[str, Any]) -> None:
    ...
    if not valid: raise ValidationError("; ".join(errors))
