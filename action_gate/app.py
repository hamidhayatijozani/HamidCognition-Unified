...
def verify_approval_signature(approval: Approval) -> bool:
    ...
    if not secret:
        return False
    expected = hmac.new(secret.encode(), canonical(approval_payload(approval)).encode(), hashlib.sha256).hexdigest()
...
def approve(decision_id: str, approval: Approval, authorization: str | None = Header(default=None)):
    ...
    if ENVIRONMENT == "production" and not APPROVAL_SECRET:
        raise HTTPException(503, "production_approval_secret_not_configured")
    if ENVIRONMENT == "production" and not verify_approval_signature(approval):
