...
def adversarial_ingress(client: httpx.Client) -> dict:
    ...
    invalid_json = client.post("/v1/csg/decide", content=b"{not-json", headers={**headers, "Content-Type": "application/json", "Idempotency-Key": "accept-invalid-json"})
    assert invalid_json.status_code == 400, invalid_json.text

    bad_signature = client.post("/v1/csg/decide", json=base, headers={**headers, "Idempotency-Key": "accept-bad-signature", "X-HCJ-Request-Signature": "0" * 64})
