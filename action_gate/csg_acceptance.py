...
def expected(action: str, target: str | None) -> str:
    if action in {"transfer_funds", "transfer_money"}:
        return "SANDBOX"
    if action in {"delete_file", "delete_customer", "delete_database"} and target and "prod" in target.lower():
        return "DENY"
...
def adversarial_ingress(client: httpx.Client) -> dict:
    ...
    headers = {
        ...
        "Idempotency-Key": base["request_id"],
        "X-HCJ-Request-Signature": sign(base),
    }

    malformed = dict(base)
    malformed.pop("contract_version")
    malformed_response = client.post("/v1/csg/decide", json=malformed, headers=headers)
    assert malformed_response.status_code == 422, malformed_response.text

    invalid_json = client.post(
        "/v1/csg/decide",
        content=b"{not-json",
        headers={**headers, "Content-Type": "application/json", "Idempotency-Key": "accept-invalid-json"},
    )
    assert invalid_json.status_code == 400, invalid_json.text

    bad_signature = client.post(
        "/v1/csg/decide",
        json=base,
        headers={**headers, "Idempotency-Key": "accept-bad-signature", "X-HCJ-Request-Signature": "0" * 64},
    )
    assert bad_signature.status_code == 401, bad_signature.text

    stale = dict(base)
    stale["request_id"] = "accept-stale"
    stale["timestamp"] = (datetime.now(timezone.utc) - timedelta(hours=1)).isoformat().replace("+00:00", "Z")
    stale_headers = {
        "Authorization": f"Bearer {TOKEN}",
        "Idempotency-Key": stale["request_id"],
        "X-HCJ-Request-Signature": sign(stale),
    }
    stale_response = client.post("/v1/csg/decide", json=stale, headers=stale_headers)
    assert stale_response.status_code == 408, stale_response.text

    first = client.post("/v1/csg/decide", json=base, headers=headers)
    assert first.status_code == 200, first.text
    changed = dict(base)
    changed["parameters"] = {"index": 9999}
    changed["request_id"] = base["request_id"]
    changed_headers = {**headers, "X-HCJ-Request-Signature": sign(changed)}
    conflict = client.post("/v1/csg/decide", json=changed, headers=changed_headers)
    assert conflict.status_code == 409, conflict.text

    other_tenant = event(9000, tenant="tenant-other")
    other_tenant["request_id"] = "accept-9000-other-tenant"
    other_headers = {
        "Authorization": f"Bearer {TOKEN}",
        "Idempotency-Key": other_tenant["request_id"],
        "X-HCJ-Request-Signature": sign(other_tenant),
    }
    isolated = client.post("/v1/csg/decide", json=other_tenant, headers=other_headers)
    ...
    with httpx.Client(base_url=BASE_URL, timeout=5.0) as client:
        ...
        for i in range(count):
            ...
            headers = {
                ...
                "X-HCJ-Request-Signature": sign(body),
            }
            ...
            expected_decision = expected(body["action"], body["target"])
            if result["decision"] == "ALLOW" and expected_decision != "ALLOW":
                false_allow += 1
        ...
        # True Decision Replay: consume the stored decision/evidence by decision_id.
        # This does not re-submit the action, does not consume the nonce, and does not
        # claim to reconstruct external world state.
        for body, original in records:
            started = time.perf_counter()
            replay = client.get(
                f"/v1/replay/{original['decision_id']}",
            ...
            if not replay_result.get("match"):
                replay_failures += 1
            digest_mismatches += not replay_result.get("policy_hash_match", False)
            digest_mismatches += not replay_result.get("action_hash_match", False)
            decision_mismatches += replay_result.get("replayed_decision") != original["decision"]
