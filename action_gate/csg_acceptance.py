...
def event(index: int) -> dict:
    ...
    return {
        ...
        "timestamp": datetime.now(timezone.utc).isoformat(), "parameters": {"index": index}, "context": {"suite": "csg-acceptance"},
    }
...
def run(count: int = 200) -> dict:
    ...
    with httpx.Client(base_url=BASE_URL, timeout=5.0) as client:
        ...
        for i in range(count):
            ...
            if result["decision"] == "ALLOW" and expected(body["action"], body["target"]) != "ALLOW":
                false_allow += 1
            expected_digest = hashlib.sha256(canonicalize(body)).hexdigest()
            digest_mismatches += result["request_digest"] != expected_digest
            score_mismatches += result["decision"] != expected(body["action"], body["target"])
            records.append((body, headers, result))

        for body, headers, original in records:
            started = time.perf_counter()
            replay = client.post("/v1/csg/decide", json=body, headers=headers)
            replay_timings.append((time.perf_counter() - started) * 1000)
            assert replay.status_code == 200, replay.text
            result = replay.json()
            digest_mismatches += result["request_digest"] != original["request_digest"]
            score_mismatches += result["decision"] != original["decision"]
    ...
    return {
        ...
        "decision_mismatches": score_mismatches,
        "false_allow": false_allow,
        "replay_exact_match": digest_mismatches == 0 and score_mismatches == 0,
    }
