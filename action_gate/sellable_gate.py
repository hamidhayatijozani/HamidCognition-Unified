...
def main() -> int:
    ...
    try:
        ...
        approval = {
            ...
            "approval_signature": __import__("hmac").new(ENV["ACTION_GATE_APPROVAL_SECRET"].encode(), __import__("json").dumps({k:v for k,v in {"approver_id":"human-1","approved":True,"reason":"sellable-gate approval test","action_hash":ask["action_hash"],"tenant_id":ask["tenant_id"],"policy_version":ask["policy_version"],"ttl_seconds":None}.items() if k!="approval_signature"}, sort_keys=True, separators=(",",":"), ensure_ascii=False).encode(), __import__("hashlib").sha256).hexdigest(),
        }
