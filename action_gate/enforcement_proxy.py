...
def reserve_execution(decision_id: str, tenant_id: str, action_hash_value: str, nonce: str):
    return post_json(GATE_URL + f"/v1/action/{urllib.parse.quote(decision_id, safe='')}/execution/reserve",
                     {"tenant_id": tenant_id, "action_hash": action_hash_value, "nonce": nonce})[1]
    ...
    def do_POST(self):
        try:
            ...
            if decision_id:
                ...
                reserve_execution(decision_id, tenant_id, expected_hash, record["nonce"])
...
class MCPHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            ...
            reserve_execution(result["decision_id"], tenant_id, result["action_hash"], result["nonce"])
